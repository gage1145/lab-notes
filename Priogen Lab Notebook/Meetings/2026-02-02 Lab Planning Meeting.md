---
name: 2026-02-02 Lab Planning Meeting
date: 2026-02-02
tags: [meeting]
continued_from: "[[2026-01-20 Lab Planning Meeting]]"
association: Priogen
project:
summary: The meeting focused on project coordination and outstanding logistics, including a lack of response from Hunter and uncertainty around swab supplies, with PBS identified as a fallback. Updates on Iceland confirmed acceptance of the estimate, shipment plans for 214 oral swabs, required protocol updates, and a projected 10 plates over ~2.5 weeks. Mississippi and MNPROtein orders remain pending timelines, though packages are prepared, and LN pooling was discussed as most viable in low-prevalence, non-endemic areas due to break-even constraints. PriLIMS was noted as an ongoing consideration without specific updates.
---

## **Attendees**
- Roxy
- Marc
- Gage

## Overview
1. Have not heard back from Hunter
	1. Swabs and potential supplies is unknown
	2. Can just send PBS
2. Iceland
	1. Next shipment of things
	2. We will need to send up-to-date protocols
	3. 214 oral swabs
	4. They accepted the estimate
	5. Marc sent shipping permit
	6. 10 plates, 2$\frac{1}{2}$ weeks
3. Mississippi order 1025
	1. Feeder and oral swabs
	2. We are still waiting on their timeline
4. MNPROtein
	1. A&M - CFIA 1130
	2. Spain - still waiting, they have an estimate
	3. All packages are ready to go
5. LN pooling
	1. For pooling, we need a prevalence of 0.75% just to break even
	2. This means it would be most ideal for areas where prevalence is very low. Target non-endemic areas.
6. PriLIMS